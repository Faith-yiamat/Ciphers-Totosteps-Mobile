...
class AutismUploadViewModel : ViewModel() {
    ...
    fun uploadImage(childId: Int, file: File) {
        viewModelScope.launch {
            _uploadState.value = UploadState.Loading
            try {
                ...
                val response = ApiClient.instance.uploadImage(childId, body)
    ...
    private fun handleResponse(response: Response<AutismTestResult>) {
        if (response.isSuccessful && response.body() != null) {
            _uploadState.value = UploadState.Success(response.body()!!)
        } else {
            _uploadState.value = UploadState.Error(response.errorBody()?.string() ?: "Unknown error occurred")
        }
    }
}
...
sealed class UploadState {
    object Idle : UploadState()
    object Loading : UploadState()
    data class Success(val response: AutismTestResult) : UploadState()
    data class Error(val message: String) : UploadState()
}